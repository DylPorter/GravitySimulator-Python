...
class PopUp:
    ...
    def make_active(self, popup_type, clicked_object):
        self.__is_active = True
        ...
        if popup_type == "tooltip":
            ...
            self.__size.x = 300
            self.__size.y = 300
            self.__pos.x = pyray.get_screen_width()/2 - 50
            self.__pos.y = pyray.get_screen_height()/2 - 50

    def make_inactive(self):
        self.__is_active = False
        self.__object.change_colour(pyray.WHITE)

    def box_click(self, point):
        if self.__is_active:
            self.__child_box.click(point)
            if not self.__child_box.get_active():
                self.__object.set_new_radius(self.__child_box.get_value())
